...
export const handleClerkUserWebhook = httpAction(async (ctx, request) => {
    ...
    switch (event.type) {
        ...
        case "user.updated": {
            ...
            const upsertResult = await ctx.runMutation(internal.modules.user.mutations.upsertFromClerk, {
                externalId: id,
        ...
        case "user.deleted": {
            ...
            const deleteResult = await ctx.runMutation(internal.modules.user.mutations.markAsDeleted, {
                externalId,
            });
