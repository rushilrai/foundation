...
export const upsertFromClerk = internalMutation({
    ...
    handler: async (ctx, args): Promise<{ userId: Id<"users"> } | { error: string }> => {
...
export const markAsDeleted = internalMutation({
    ...
    handler: async (ctx, args): Promise<{ deleted: true; alreadyDeleted?: boolean } | { error: string }> => {
        ...
        if (!user) {
            console.warn("markAsDeleted called for non-existent user", { externalId: args.externalId });
