...
export const getById = async (ctx: QueryCtx | MutationCtx, patchId: Id<"patches">) => {
...
export const getByIdWithAuth = async (
    ...
    patchId: Id<"patches">
): Promise<{ patch: Doc<"patches">; user: Doc<"users"> } | { error: string }> => {
...
export const getPatchesForResume = async (
    ...
    const patches = await ctx.db
        .query("patches")
        .withIndex("by_resumeId_deleted", (q) => q.eq("resumeId", resumeId).eq("deleted", false))
        .order("desc")
        .collect();
...
export const getUserPatches = async (ctx: QueryCtx | MutationCtx, userId: Id<"users">) => {
    const patches = await ctx.db
        .query("patches")
