...
import { getById, getByIdWithAuth, getPatchesForResume, getUserPatches } from "./helpers";

export const listForResume = query({
    args: { resumeId: v.id("resumes") },
    handler: async (ctx, args): Promise<{ patches: Array<Doc<"patches">> } | { error: string }> => {
...
export const list = query({
    args: {},
    handler: async (ctx): Promise<{ patches: Array<Doc<"patches">> } | { error: string }> => {
        const identity = await ctx.auth.getUserIdentity();
        if (!identity) {
            return { error: "UNAUTHORIZED" };
        }
...
export const get = query({
    ...
    handler: async (ctx, args): Promise<{ patch: Doc<"patches"> } | { error: string }> => {
