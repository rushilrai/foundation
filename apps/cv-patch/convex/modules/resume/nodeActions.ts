...
export const extractResumeData = internalAction({
    ...
    handler: async (ctx, args): Promise<void> => {
        try {
            ...
            if (!resume) {
                console.error("[extractResumeData] Resume not found", { resumeId: args.resumeId });
                return;
            }
...
extras: string[]`;
            ...
            if (issues.length > 0) {
                ...
                if (retryOutput) {
                    data = retryOutput as ResumeData;
                }
            }

            const finalIssues = validateResumeData(data);
            if (finalIssues.length > 0) {
                throw new Error(`Resume data failed validation: ${finalIssues.join("; ")}`);
            }
