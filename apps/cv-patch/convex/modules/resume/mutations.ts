...
export const create = mutation({
    ...
    handler: async (ctx, args): Promise<{ resumeId: Id<"resumes"> } | { error: string }> => {
        ...
        try {
            ...
            await ctx.scheduler.runAfter(0, internal.modules.resume.nodeActions.extractText, {
                resumeId,
            });
...
export const updateExtractedContent = internalMutation({
    ...
    handler: async (ctx, args): Promise<void> => {
        await ctx.db.patch(args.resumeId, {
            extractedXml: args.extractedXml,
            status: args.status,
